...
class AppStore extends Component{
    state = {currentTab:tabsList[0].tabId, searchValue:""};
    ...
    render(){
        ...
        const filteredappList = appsList.filter((eachItem)=>eachItem.category===currentTab);
        const SearchedList = filteredappList.filter((eachItem)=> eachItem.appName.toLowerCase().includes(searchValue.toLowerCase()));
        return(
            <div className={'container1'}>
                <h2 className={'head'}>App Store</h2>
                <div className={'searchContainer'}>
                    <input className={'searchInput'} value={searchValue} onChange={this.SearchedValue} type={'search'} placeholder={'Search'}></input>
                </div>
                <ul className={'buttonsContainer'}>
                    {tabsList.map((eachItem)=>{
                       return <ButtonCom tabs = {eachItem} key={eachItem.tabId} getTabId = {this.getTabId} currentTab={currentTab}/>
                   })}
               </ul>
                <div className={"itemscontainer"}>
               <ul className={'unorderlist'}>
                   {SearchedList.map((eachItem)=>{
                     return  <AppItem apps = {eachItem} key = {eachItem.appId}/>
                   })}
               </ul>
